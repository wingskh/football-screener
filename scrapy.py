    ...
    while True:
        try:
            ...
            for betting_company in betting_companies:
                ...
                soup_collection[betting_company] = BeautifulSoup(
...
def format_recent_df(selected_recent_data_df, home_team):
    selected_recent_data_df["主紅牌"] = selected_recent_data_df["主場"].str.extract(
        r"(\d+)", expand=False
    )
    selected_recent_data_df["主紅牌"] = selected_recent_data_df["主紅牌"].fillna(0)
    selected_recent_data_df["主場"] = (
        selected_recent_data_df["主場"].str.replace(r"\d+", "", regex=True).str.strip()
    )
    selected_recent_data_df["客紅牌"] = selected_recent_data_df["客場"].str.extract(
        r"(\d+)", expand=False
    )
    selected_recent_data_df["客紅牌"] = selected_recent_data_df["客紅牌"].fillna(0)
    selected_recent_data_df["客場"] = (
        selected_recent_data_df["客場"].str.replace(r"\d+", "", regex=True).str.strip()
    )
    selected_recent_data_df["場地"] = "客"
    selected_recent_data_df.loc[
        selected_recent_data_df["主場"].str.contains(home_team), "場地"
    ] = "主"
    selected_recent_data_df.loc[
        selected_recent_data_df["主場"].str.contains("(中)"), "場地"
    ] = "中"
...
def get_recent_data(match_id):
    ...
    ranking_df = {}
    try:
        ranking_tables = driver.find_elements(
            By.XPATH, '//*[@id="porlet_5"]/div/table/tbody/tr[1]//tbody'
        )
        if len(ranking_tables) >= 2:
            ...
            for team in ranking_df:
                ...
                team_data = [
                    [td.text for td in data.find_elements(By.TAG_NAME, "td")]
        ...
        try:
            ranking_tables = driver.find_elements(
                By.XPATH, '//*[@id="porlet_5"]/div/table/tbody/tr[1]//tbody//tbody'
            )
            if len(ranking_tables) >= 2:
                ranking_df["home"] = ranking_tables[0]
                ranking_df["away"] = ranking_tables[1]
    ...
    try:
        ...
        for company in selected_company_list:
            ...
            vs_data = [
                [data.text for data in vs_data.find_elements(By.TAG_NAME, "td")]
                for vs_data in vs_table.find_elements(By.TAG_NAME, "tr")
            ][2:]
            if len(vs_data) > 2:
    ...
    try:
        odds_providers = {
            "home": {
                "selection_box": Select(
                    driver.find_element(By.XPATH, '//*[@id="hSelect_hn"]')
                ),
                "table_id": "table_hn",
            },
            "away": {
                "selection_box": Select(
                    driver.find_element(By.XPATH, '//*[@id="hSelect_an"]')
                ),
                "table_id": "table_an",
            },
        }
        for team in odds_providers:
            ...
            for company in selected_company_list:
                odds_providers["home"]["selection_box"].select_by_value(
                    company_to_info[company]["company_id"]
                )
                recent_table = driver.find_element(
                    By.XPATH, f'//*[@id="{odds_providers[team]["table_id"]}"]'
                )
                selected_recent_data = [
                    [data.text for data in recent_data.find_elements(By.TAG_NAME, "td")]
                    for recent_data in recent_table.find_elements(By.TAG_NAME, "tr")
                ]
                selected_recent_data_df = None
                if len(selected_recent_data) > 2:
                    selected_recent_data = [
                        selected_recent_data[0][:6] + selected_recent_data[1]
                    ] + selected_recent_data[2:]
                    selected_recent_data_df = pd.DataFrame(
                        selected_recent_data[1:-1], columns=selected_recent_data[0]
                    )
                    recent_matches_df_dict[company] = format_recent_df(
                        selected_recent_data_df, home_team
                    )
...
def get_had_df(match_id):
    ...
    columns = [
        ...
        "unmasked_company_name",
        "unknown1",
        "unknown2",
    ]
    had_df = pd.DataFrame([x.split("|") for x in game], columns=columns)
    had_df["unmasked_company_name"] = had_df["unmasked_company_name"].str.replace(
        r"\([^)]+\)", "", regex=True
    )
    ...
    cid_to_company = (
        had_df[["masked_company_name", "cid"]].set_index("cid").to_dict("index")
    )
    cid_to_company = {
        k: v["masked_company_name"]
        for k, v in cid_to_company.items()
        if v["masked_company_name"] in company_to_info
    }
    had_df["company_id"] = had_df["masked_company_name"].map(
        lambda x: company_to_info[x]["company_id"] if x in company_to_info else np.nan
    )
...
def get_handicap_odds(match_id, home_team, away_team, home_extra_goal, match_info=[]):
    ...
    while True:
        try:
            ...
            match_time = datetime.strptime(
                soup.find("span", {"class": "time"}).text.split("\xa0")[0],
                "%Y-%m-%d %H:%M",
            )
            break
        except Exception as e:
    ...
    if len(handicap_table_data) > 2:
        handicap_table_data = [
            [data.text for data in row.select('td:not([style*="display: none"])')]
            for row in handicap_table_data
        ]
        ...
        handicap_df = handicap_df.assign(
            numeric_init_handicap_line=handicap_df["init_handicap_line"].apply(
                convert_handicap_string_to_float
            )
        )
        handicap_df = handicap_df.assign(
            numeric_last_handicap_line=handicap_df["last_handicap_line"].apply(
                convert_handicap_string_to_float
            )
        )
        handicap_df["init_match_result"] = (
            handicap_df["numeric_init_handicap_line"] + home_extra_goal
        )
        handicap_df["last_match_result"] = (
            handicap_df["numeric_last_handicap_line"] + home_extra_goal
        )
    ...
    for company in changed_odds_url_dict:
        ...
        flatten_odds_df.columns = [
            "home_odds",
            "handicap_line",
            "away_odds",
            "time",
            "status",
            "numeric_handicap_line",
        ]
    ...
    return {
        ...
        "handicap_df": handicap_df,
        "changed_odds_dict": flatten_odds_df,
    }
...
def get_odds_list(odds_url, match_time):
    ...
    selected_trs = [
        ...
        for tr in soup.find("span", {"id": "odds2"}).find_all("tr")
    ]
    ...
    odds_table = odds_table.assign(
        numeric_handicap_line=odds_table["盘口"].apply(convert_handicap_string_to_float)
    )
...
def get_latest_odds(date):
    ...
    daily_match_data = [
        [x.text.strip() for x in row.find_all("td")] + [row["sid"]]
        for row in soup.find("table", {"id": "table_live"}).select("tr[sid]")
    ]
    if len(daily_match_data) == 0:
        temp_daily_match_data = [
            row.find_all("td")
            for row in soup.find("table", {"id": "table_live"}).select("tr[infoid]")
        ][1:]
        daily_match_data = [
            [x.text.strip() for x in row[:-1]]
            + [
                re.search(
                    r"\((.*?)\)", row[-1].select('a:contains("析")')[0]["onclick"]
                ).group(1)
            ]
            for row in temp_daily_match_data
        ]
...
def start_scrapy():
    ...
    while True:
        ...
        pickle.dump(
            matches_info, fs.open(f"s3://{s3_bucket}/{formatted_date}.pkl", "wb")
        )
